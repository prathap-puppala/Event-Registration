...
<img style='position:fixed;top:0%;height:13%;width:100%;' src='header.png'><Br><br>
...
<div  style='background:url(3.png);height:100%;'>
  <body >
	...
	{
	$("#sids").hide(3000);
	$("#tid").html("<select id='bttype' ><option selected value=''>Choose</option><option value='p1'>PUC TEAMS</option><option value='eng'>ENGG TEAMS</option></select><select id='etype' ><option selected value=''>Choose Event</option><option value='debate'>Debate</option><option value='quiz'>Quiz</option></select><br><button onclick='teamload()' class='ui-button-primary' style='height:auto;'>View Teams</button>");
	}
	function teamload()
	{
	var ttype=$("#bttype").val();
	var etype=$("#etype").val();
	$("#teams").html("<br><font color='white'>Loading<blink>...</blink></font>");
	  ...
	  function dothis()
		{
			...
			for(var i=1;i<=len;i++)
				{
					var tmp="<font color='white'>ID NO"+i+" </font><input type=text style='height:auto;' maxlength=7 id=uid"+i+" name=uid"+i+"><br>";
					str=str+tmp;
				}
  ...
  <![endif]-->
	...
	<select id='batch' onchange="dothis()">
